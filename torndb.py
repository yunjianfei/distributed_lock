...
class Connection(object):
        ...
        def call(self, *args, **kwargs):
            ...
            while attempts < 10:
                ...
                try:
                    ...
                    logging.debug("Mysql Error: %s. Retry! [attempts: %s]", str(e), attempts)
                    time.sleep(0.3)
                    self.reconnect()
                    attempts += 1
                    continue
                #    if 'MySQL server has gone away' in str(e):
                #        logging.info('MySQL server has gone away, will try to re-connect.')
                #        # re-connect MySQL
                #        self.reconnect()
                #        attempts += 1
                #    else:
                ...
                #finally:
                #    self.lock.release()
                logging.error("Error: Has retry 5 times! Already failed!") 
                return None
        return call
    ...
    def initClientEncode(self):
        ...
        curs = self._cursor()
        self._execute(curs, "SET NAMES utf8", None, None)
        return curs
    ...
    def get_fields_str(self, tablename):
        ...
        self._execute(cursor, sql, None, None)
        fields = []
    ...
    def get_select_sql_cache(self, tablename):
        str = self.get_fields_str(tablename)
        sql = "SELECT SQL_CACHE " + str + " FROM " + tablename + " "
        return sql
